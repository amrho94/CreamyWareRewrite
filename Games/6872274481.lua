...
local function isBadExecutor()
    local badList = { "codex", "ronix", "frostware", "Fluxus", "Solara", "Evon", "VegaX" }
...
run(function()
    ...
    AdetundeExploit = vape.Categories.Utility:CreateModule({
		...
		Function = function(callback)
			...
			if callback then
				Speed:Clean(runService.PreSimulation:Connect(function(dt)
					...
					local isFlyEnabled = rawget(_G, "Fly") and Fly.Enabled
					local isInfiniteFlyEnabled = rawget(_G, "InfiniteFly") and InfiniteFly.Enabled
					local isLongJumpEnabled = rawget(_G, "LongJump") and LongJump.Enabled

					if entitylib.isAlive and not isFlyEnabled and not isInfiniteFlyEnabled and not isLongJumpEnabled and isnetworkowner(entitylib.character.RootPart) then
